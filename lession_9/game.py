...
# music
pygame.mixer.music.load(os.path.join('music', 'Bionic Commando (2009) - 18 - Piano Theme.mp3'))
...
# background pics
background = pygame.image.load(os.path.join('img', 'small_irregular_cubes-wallpaper-1366x768.jpg'))
backgroundGameLoop = pygame.image.load(os.path.join('img', 'light_green_2-wallpaper-1366x768.jpg'))
# base pics
spring = pygame.image.load(os.path.join('img', 'Seasons-Spring.jpg'))
winter = pygame.image.load(os.path.join('img', 'Seasons-Winter.jpg'))
fall = pygame.image.load(os.path.join('img', 'Seasons-Fall.jpg'))
# second level additonal pics
summer = pygame.image.load(os.path.join('img', 'Seasons-Summer.jpg'))
pirates = pygame.image.load(os.path.join('img', 'Pirates.jpg'))
...
gameIcon = pygame.image.load(os.path.join('img', 'teky.png'))
...
def button(msg, x, y, w, h, ib_c, ab_c, it_c, at_c, action=None):
    ...
    if x + w > pos[0] > x and y + h > pos[1] > y:
        ...
        text(msg, x + (w / 2), y + (h / 2), 50, at_c, os.path.join('font', 'LittleLordFontleroyNF.ttf'))
        if click[0] == 1 and action != None:
            action()
    else:
        pygame.draw.rect(gameDisplay, ib_c, (x, y, w, h))
        text(msg, x + (w / 2), y + (h / 2), 50, it_c, os.path.join('font', 'LittleLordFontleroyNF.ttf'))
...
def about():
    ...
    while intro:
        ...
        text('References:', display_width / 2, display_height / 1.25, 15, white,
             os.path.join('font', 'coolvetica rg.ttf'))
        text(' - HỌC VIỆN SÁNG TẠO TRẺ TEKY, from http://www.teky.edu.vn/', display_width / 2, display_height / 1.2, 15,
             white, os.path.join('font', 'coolvetica rg.ttf'))

        button("Back", (display_width / 2) - 100, (display_height / 2) - 50, 200, 100, white, red, red, white,
               game_intro)
    ...
    while intro:
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                quit()
        gameDisplay.blit(background, (00, 00))
        text('Picture Puzzle Game', display_width / 2, display_height / 7, 50, red,
             os.path.join('font', 'crackman.ttf'))
        text('HỌC VIỆN TEKY', display_width / 2, display_height / 5, 25, white,
             os.path.join('font', 'coolvetica rg.ttf'))
...
def game_loop(level=1, oldchoosen=None, oldtile=None, old_x=None):
    ...
    while not gameExit:
        ...
        if level == 1:
            ...
            text('Level 1', display_width / 2, display_height / 8, 35, white, os.path.join('font', 'zerovelo.ttf'))
            ...
            gameDisplay.blit(backgroundGameLoop, (0, 0))
            text('Level 2', display_width / 2, display_height / 8, 35, white, os.path.join('font', 'zerovelo.ttf'))
        ...
        if tile.count(True) > x:
            if check_same(tile, choosen) == True:
                if level == 1:
                    text('Nice !!!', display_width / 2, display_height / 1.15, 30, blue,
                         os.path.join('font', 'coolvetica rg.ttf'))
                else:
                    text('Nice !!!', display_width / 2, display_height / 1.05, 30, blue,
                         os.path.join('font', 'coolvetica rg.ttf'))
                Won = True
                x += 2
            else:
                if level == 1:
                    text('Nope !!!', display_width / 2, display_height / 1.15, 30, red,
                         os.path.join('font', 'coolvetica rg.ttf'))
                else:
                    text('Nope !!!', display_width / 2, display_height / 1.05, 30, red,
                         os.path.join('font', 'coolvetica rg.ttf'))
                Won = False
