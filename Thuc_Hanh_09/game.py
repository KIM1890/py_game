...
def game_intro():
    ...
    while intro:
        ...
        button("About", (display_width/2)-100, 325,
               200, 100, white, red, red, white, about)

        button("Quit", (display_width/2)-100, 450, 200,
               100, white, red, red, white, quitgame)
...
def l_random(level=1):
    ...
    while len(l_final) != len(l[level-1])*2:
        ...
        if l_final.count(choice) <= 1:
            l_final.append(choice)
...
def game_loop(level=1, oldchoosen=None, oldtile=None, old_x=None):
    ...
    while not gameExit:
        ...
        if level == 1:
            ...
            gameDisplay.blit(pygame.transform.scale(
                choosen[1], (150, 150)), (325, 100))

            gameDisplay.blit(pygame.transform.scale(
                choosen[2], (150, 150)), (475, 100))

            gameDisplay.blit(pygame.transform.scale(
                choosen[3], (150, 150)), (175, 250))

            gameDisplay.blit(pygame.transform.scale(
                choosen[4], (150, 150)), (325, 250))

            gameDisplay.blit(pygame.transform.scale(
                choosen[5], (150, 150)), (475, 250))

            gameDisplay.blit(pygame.transform.scale(
                choosen[6], (150, 150)), (250, 400))

            gameDisplay.blit(pygame.transform.scale(
                choosen[7], (150, 150)), (400, 400))
        ...
        if level == 1:
            ...
            if not tile1:
                pygame.draw.rect(gameDisplay, white, (100, 100, 200, 200))

            if not tile2:
                pygame.draw.rect(gameDisplay, white, (300, 100, 200, 200))

            if not tile3:
                pygame.draw.rect(gameDisplay, white, (500, 100, 200, 200))

            if not tile4:
                pygame.draw.rect(gameDisplay, white, (100, 300, 200, 200))

            if not tile5:
                pygame.draw.rect(gameDisplay, white, (300, 300, 200, 200))

            if not tile6:
                pygame.draw.rect(gameDisplay, white, (500, 300, 200, 200))

        elif level == 2:

            if not tile1:
                pygame.draw.rect(gameDisplay, white, (175, 100, 150, 150))

            if not tile2:
                pygame.draw.rect(gameDisplay, white, (325, 100, 150, 150))

            if not tile3:
                pygame.draw.rect(gameDisplay, white, (475, 100, 150, 150))

            if not tile4:
                pygame.draw.rect(gameDisplay, white, (175, 250, 150, 150))

            if not tile5:
                pygame.draw.rect(gameDisplay, white, (325, 250, 150, 150))

            if not tile6:
                pygame.draw.rect(gameDisplay, white, (475, 250, 150, 150))

            if not tile7:
                pygame.draw.rect(gameDisplay, white, (250, 400, 150, 150))

            if not tile8:
                pygame.draw.rect(gameDisplay, white, (400, 400, 150, 150))
